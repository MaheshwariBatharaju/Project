...
import com.smart.helper.Messsage;
...
//	this handler for registering user
	...
	,defaultValue="false") boolean agreement,Model model,  HttpSession session) {
		...
		try {
		    ...
		    model.addAttribute("user",new User());
		    session.setAttribute("message", new Messsage("successfully registered!!","alert-success"));
		    return "signup";
		...
		e.printStackTrace();
		model.addAttribute("user", user);
		session.setAttribute("message", new Messsage("something went wrong!!"+e.getMessage(),"alert-danger"));
	    return "signup";
		}
	}
